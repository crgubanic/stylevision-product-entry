...
ALWAYS use the field Product Name early in the description.
Vary your introduction style (e.g, do not always use "Experience the vibrant...", or "Perfect for...", or "Introducing the..."), but ALWAYS include the field Product Name.
Clearly mention washing/care instructions last; "Cool Iron" and "Warm Iron" should not be "Cooling Iron" nor "Warming Iron".
Combine repetitive attributes naturally (e.g., "Red and Blue" instead of "Red, Blue, Red", "machine wash warm water" instead of "wash in machine wash with warm water").
...
Add a little marketing flair, but ensure the Product Name is featured prominently.
You MUST use ALL the attributes below, plus the above, to create a flowing paragraph:
...
if st.button("Generate Description"):
    if not (name.strip() and products and brand.strip() and fabric and uploaded_file and colour != "-- Select Colour --"):
        ...
        with st.spinner("Generating awesome description..."):
...
if st.button("Save Product", disabled=st.session_state.get("saving", False)):
    ...
    with st.spinner("Saving your product..."):
        ...
        if missing_fields:
            ...
            st.success(f"Product '{st.session_state.get('name', '')}' saved successfully with ID {st.session_state['p_id']}!")
